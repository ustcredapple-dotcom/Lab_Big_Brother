...
Lark runs through the official `lark-oapi` Python SDK and WebSocket events, with an OpenAPI polling fallback enabled by default, so it does not need a public ngrok callback. A working `Receive message` event should wake the bot immediately on `@`; polling is only the resilience path when events are not delivered. The default fallback message poll interval is 5 seconds, while bot identity is cached for 5 minutes and the visible chat list is cached for 1 minute. Once the bot is added to a group, it records delivered or polled group messages by default under `/Volumes/ZZLab_AI/YYYY-MM-DD/lark文档和消息记录/<sender>_<chat>/`. When a new group first becomes visible while the service is already running, `backfill_new_chats=true` makes the bot silently page through accessible history, archive messages, and download message attachments/files/images without replying to old mentions. `backfill_new_chat_max_messages=0` means best-effort all accessible history, subject to Lark permissions and retention. To receive every message in a group instead of only direct mentions, the Lark app should still be granted the official scope `Read all messages in associated group chat` and subscribe to the `Receive message` event. In group chats it should remain quiet unless mentioned with `@大师兄` or called with explicit commands such as `/ask`, `/note`, `/help`, `/status`, or `/id`. In private chat it may answer directly when events are delivered or when the p2p chat is visible to polling / listed in `polling_extra_chat_ids`. Files and images are archived automatically; PDFs/text-like files get text/HTML previews, images get Qwen vision previews, and binary engineering files are stored as metadata-only attachments.
