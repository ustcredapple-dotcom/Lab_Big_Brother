...
Telegram behavior is intentionally short-form: greetings and light social messages get a natural “大师兄已就位” style reply, while normal lab questions query the notebook. `记` or `/note` writes a note, `开始记` switches that chat into record mode, and `停止记` switches back to query mode. Authorized users can also run small admin actions such as `/allow chat_id` or “把 8004894761 写进白名单”. Query replies use DeepSeek at runtime to inspect the distilled notebook directory, select evidence pages, and write the short answer; if DeepSeek finds no direct evidence, the bot should say it does not know instead of forcing an unrelated result. Query replies can include an HTML detail file. Any actual uploaded file is archived automatically by date and sender, using the caption/current chat as context; text-like files, Markdown, HTML, CSV/JSON, and PDF files get text/HTML previews, while binary engineering files such as STEP or EXE are stored as metadata-only attachments.
