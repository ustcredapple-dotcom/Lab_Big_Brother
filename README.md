...
Telegram and email daily digests keep their raw archives by source, but DeepSeek memory indexing is topic-first. The nightly digest classifies new communication records against existing notebook sections such as equipment purchasing, ARTIQ, and the Yb experiment, then writes supplemental pages under those topics. Only records that cannot be confidently matched go to `Unsorted Communication Records`; `Telegram Records` and `Email Records` are provenance channels, not long-term topic buckets.
