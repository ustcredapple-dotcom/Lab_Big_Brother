...
Telegram and email are source channels, not long-term memory topics. Their daily digest scripts should keep raw archives in source folders for auditability, then call `topic_distillation.py` so DeepSeek classifies each durable record into an existing notebook topic section when possible. Only records with no confident topic match should go to `Unsorted Communication Records`. Do not create new `Telegram Records` or `Email Records` pages in the distilled index for routine memory.
