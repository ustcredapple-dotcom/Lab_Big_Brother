...
Telegram replies should stay brief and alive. The Telegram entrypoint now has a small local agent backend: explicit commands run directly, while other normal messages go through a DeepSeek router that may select only from a safe tool registry (`chat`, `query_notebook`, `note`, `find_file`, `allow_chat_id`, `help`, `status`, or `unsupported`). It must not expose arbitrary shell or unrestricted filesystem access. Greetings, thanks, and simple presence checks should be handled by the Telegram personality layer instead of being sent to notebook search. Safe admin intents, such as adding a chat ID to the allow-list, should route to explicit local tools instead of notebook search. Runtime Telegram queries should call DeepSeek on the distilled notebook directory to select evidence pages, then call DeepSeek again on the selected page details to write the short answer. Python may load files and assemble prompts, but it should not decide an answer from keyword scores alone. If DeepSeek finds no direct evidence, answer naturally that the notebook does not contain a clear record. Query details are written to an HTML file and sent back as a Telegram document when evidence exists. Any actual Telegram file upload is archived automatically under `/Volumes/ZZLab_AI/YYYY-MM-DD/telegram文件和聊天记录/<person>_<chat_id>/`, using its caption/current chat as context. PDF and text-like files are extracted into text/HTML previews, while binary files such as STEP or EXE are stored without content extraction.
