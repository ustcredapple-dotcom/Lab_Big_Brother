...
Lark 使用国际版 `https://open.larksuite.com`，主通道是官方 WebSocket 事件连接，同时启用了 OpenAPI 主动轮询兜底，不需要 ngrok。正常情况下，`Receive message` 事件应该在你 @ 它时立刻唤醒 bot；轮询只是事件不送达或延迟时的兜底。当前默认消息轮询间隔是 5 秒，bot 身份缓存 5 分钟，群列表缓存 1 分钟，避免过度刷 API。bot 被拉进群后默认记录收到或轮询到的群消息；群里只有 @ 大师兄或使用命令时才回复。服务运行期间首次发现新群时，`backfill_new_chats=true` 会让大师兄静默分页回填该群可访问历史，归档发言记录，并下载消息附件、文件、图片、PDF/文本类文档；历史 @ 不会触发回复。`backfill_new_chat_max_messages=0` 表示尽力拉取全部可访问历史，但 Lark API 权限和消息保留策略仍决定最多能读到多早。私聊在事件送达时可直接回复；如果事件不送达，则需要 Lark API 能暴露该 p2p 会话，或把已知会话 ID 放进 `polling_extra_chat_ids`。
