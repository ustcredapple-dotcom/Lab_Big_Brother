...
Lark uses the official WebSocket event connection and also runs an OpenAPI polling fallback. It does not need ngrok. A working `Receive message` event should wake the bot immediately on `@`; the polling fallback exists for cases where Lark events are delayed or not delivered. The default fallback message poll interval is 5 seconds, with bot identity cached for 5 minutes and the chat list cached for 1 minute to keep API load reasonable. Once the bot is added to a group, it records delivered or polled group messages by default under `/Volumes/ZZLab_AI/YYYY-MM-DD/lark文档和消息记录/`. When a new group first becomes visible while the service is already running, `backfill_new_chats=true` makes the bot silently page through the accessible group history, archive messages, and download message attachments/files/images without replying to old mentions. `backfill_new_chat_max_messages=0` means best-effort all accessible history; Lark API permissions and retention still determine how far back the bot can read. To receive every message in a group instead of only direct mentions, the Lark app should still be granted the official scope `Read all messages in associated group chat` and subscribe to the `Receive message` event. In group chats the bot stays quiet unless mentioned with `@大师兄` or called with commands such as `/ask`, `/note`, `/help`, `/status`, or `/id`. In private chat it can reply directly when events are delivered or when the p2p chat is visible/added to `polling_extra_chat_ids`. Uploaded files and images are archived like Telegram files: text-like files and PDFs get extracts, images get Qwen vision previews, and binary engineering files are stored as metadata-only attachments. The nightly Lark digest is topic-first and should supplement existing notebook sections instead of creating a durable `Lark Records` topic.
...
`tenant_access_token_ok=true` means the App ID/App Secret are valid. `bot_enabled=true` is also required before the app can be added to chats, receive message events, and reply as 大师兄. If Lark returns `app do not have bot`, enable the Bot capability in the Lark developer console and release/publish the app again.
