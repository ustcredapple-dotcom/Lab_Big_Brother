...
bot 被拉进群后默认记录可收到或轮询到的群消息。服务运行期间首次发现新群时，`backfill_new_chats=true` 会静默分页回填该群可访问历史，归档发言记录，并下载消息附件、文件、图片、PDF/文本类文档；历史 @ 不会触发回复。`backfill_new_chat_max_messages=0` 表示尽力拉取全部可访问历史，但 Lark API 权限和消息保留策略仍决定最多能读到多早。群里只有 @ 大师兄或显式命令时才回复。私聊在事件送达时可以直接回复；如果事件不送达，则需要 Lark API 能暴露该 p2p 会话，或把已知会话 ID 放进 `polling_extra_chat_ids`。

在线诊断：

```bash
python /Volumes/ZZLab_AI/AI_Agent/Lab_Memory_Agent/skills/lab-senior-brother/scripts/lark_lab_senior_brother.py --check-online
```

`tenant_access_token_ok=true` 说明凭据正确；`bot_enabled=true` 才表示机器人能力真的可用。如果返回 `app do not have bot`，即使权限已经 approve，也还需要在 Lark developer console 里启用 Bot/机器人能力并重新发布应用。
